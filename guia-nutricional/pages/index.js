...
` }} />
  ...
  if (typeof window !== 'undefined') {
    setTimeout(() => {
      ...
      function initPopup(){
        document.querySelectorAll('a[href="https://pay.kiwify.com.br/y8GYnfg"]').forEach(function(btn){
          btn.removeEventListener('click', window._abrirPopup);
          window._abrirPopup = function(e){
            e.preventDefault();
            var popup = document.getElementById('leadPopup');
            if(popup) popup.classList.add('show');
          };
          btn.addEventListener('click', window._abrirPopup);
        });
      }
      initPopup();
      setTimeout(initPopup, 1000);
...
function irParaPagamento(){
  fecharPopup();
  window.open('https://pay.kiwify.com.br/y8GYnfg', '_blank');
}
